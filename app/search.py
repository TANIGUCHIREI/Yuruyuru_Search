...
with open("./data_for_search/selected_categories_list.pickle","rb") as f:
    category_list = pickle.load(f)

with open("./data_for_search/actor_list.pickle","rb") as f:
    full_actor_list = pickle.load(f)

with open("./data_for_search/full_place_list.pickle","rb") as f:
    full_place_list = pickle.load(f)

with open("./data_for_search/full_theme_list.pickle","rb") as f:
    full_theme_list = pickle.load(f)
...
def results():
    ...
    # print(AndOr)
    _is_from_manga_page = True if not request.args.get('start_year') else False #漫画ページからカテゴリなどでresultに遷移した場合はここがTrueになる。この場合はyearなどが定義されていないので設定しなくてはいけない
    search_init_year=int(request.args.get('start_year')) if request.args.get('start_year') else 1900
    search_final_year=int(request.args.get('end_year')) if request.args.get('end_year') else 2024
    manga= True if request.args.get("manga") or _is_from_manga_page else False
    anime=True if request.args.get("anime") or _is_from_manga_page else False
    novel= True if request.args.get("novel") or _is_from_manga_page else False
    ...
    return_results = search_database(
        ...
        Use_GPT = use_gpt,
        Use_GTP_Query = True,
        search_init_year=search_init_year,
        search_final_year =search_final_year,
